...
class ModuleBarrelExposesOnlyPublicApi extends DartLintRule {
  ...
  static const _code = LintCode(
      name: 'module_barrel_exposes_only_public_api',
      problemMessage:
          'Module barrel may only export application/, infrastructure/ (DI providers), presentation/routes.dart, domain/repositories/read_*.dart, domain/events/, domain/value_objects/. Do not export screens.');
  ...
  void run(CustomLintResolver resolver, ErrorReporter reporter, CustomLintContext context) {
    ...
    context.registry.addExportDirective((node) {
      ...
      final allowed = uri.contains('application/') ||
          uri.contains('infrastructure/') ||
          uri.contains('presentation/routes') ||
          uri.contains('domain/repositories/read_') ||
          uri.contains('domain/events/') ||
          uri.contains('domain/value_objects/');
