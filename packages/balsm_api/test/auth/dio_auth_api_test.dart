...
void main() {
  test('verifyOtp posts snake_case body and parses the enveloped token response', () async {
    ...
    final res = await api
        .verifyOtp(const VerifyOtpRequest(email: 'a@b.c', code: '123456', deviceId: 'd1', deviceLabel: 'iPhone'));
  ...
  test('423 lockout carries account_locked code and Retry-After', () {
    final adapter = FakeHttpAdapter((_) => jsonResponse('{"code": "account_locked"}', status: 423, headers: {
          'Retry-After': ['120']
        }));
    final api = DioAuthApi(net: fakeNet(adapter));
    expect(
  ...
  test('refresh and recoveryClaim parse the enveloped refreshed tokens', () async {
    final adapter = FakeHttpAdapter((_) => jsonResponse('{"data": {"access_token": "at2", "refresh_token": "rt2"}}'));
    final api = DioAuthApi(net: fakeNet(adapter));

    final r1 = await api.refresh(const RefreshTokenRequest(refreshToken: 'rt', deviceId: 'd'));
    expect(r1.accessToken, 'at2');

    final r2 = await api.recoveryClaim(
        const RecoveryClaimRequest(recoveryToken: 'rec', newEmail: 'n@b.c', deviceId: 'd', deviceLabel: 'l'));
