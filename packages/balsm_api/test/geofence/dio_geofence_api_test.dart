...
void main() {
  test('parses enveloped denied_codes', () async {
    final adapter = FakeHttpAdapter((_) => jsonResponse('{"data": {"denied_codes": ["kp", " ir "]}}'));
    final res = await DioGeofenceApi(net: fakeNet(adapter)).getDeniedCountries();
  ...
  test('falls back to flat body when data missing (legacy tolerance)', () async {
    final adapter = FakeHttpAdapter((_) => jsonResponse('{"denied_codes": ["kp"]}'));
    final res = await DioGeofenceApi(net: fakeNet(adapter)).getDeniedCountries();
    expect(res.deniedCodes, ['kp']);
  });

  test('non-list denied_codes → empty', () async {
    final adapter = FakeHttpAdapter((_) => jsonResponse('{"data": {"denied_codes": "oops"}}'));
    final res = await DioGeofenceApi(net: fakeNet(adapter)).getDeniedCountries();
