...
```
packages/balsm_api/
  pubspec.yaml            # pure Dart: dio, freezed_annotation, json_annotation
  lib/
...
- Dev deps: `build_runner`, `freezed`, `json_serializable`, `test`, `mocktail` (client tests stub `HttpClientAdapter`, matching the repo's existing mocktail convention).

## DTO conventions

- freezed classes with `fromJson`/`toJson`.
- Wire names are snake_case via `@JsonKey(name: '...')` (matches existing .NET payloads: `country_code`, `device_id`, …).
- Requests and responses only — **no domain types in this package**. Modules keep DTO→domain mapping in their use cases/adapters (anti-corruption stays in the bounded context).
...
6. **disclosure / geofence_block / medications** — remainder sweep
...
- Update `balsm_boundary_lint` allowlist source even though the package is currently excluded from melos (analyzer-version pin); rule text must not rot.
