...
| `feedback.jsx` | `aria-pressed` ported as a per-mark label; the uniform-gold lit mark and the "N of 5" line **deliberately not followed** — see below |
...
### Two of `feedback.jsx`'s rating changes are deliberately not followed

Both were ported, then reverted on request.

**The uniform gold.** The design flattens a lit mark to one gold (#F0AE1A,
"no two-tone, no sweep"). A lit mark keeps the brand's five hues instead: the
rating row is the one place that mark earns its keep, and gold trades it for a
star metaphor the mark is not.

**The "N of 5" line.** The design prints the picked score in mono under the
row. Removed — the word label (Rough … Great) already says what was picked,
and a number beside it reads as a second, competing readout.

What IS followed: the unlit flat ink wash, and the per-mark accessible label
the design added `aria-pressed` for.

Standing divergences, not oversights. `_RateMark` carries the reasoning and
the design's gold value; the `fb_rate_count` key was deleted from both bundles
